...
%
% 2026-09-04: caption cut from 261 words to ~200, author-directed -- it was
% more than twice the length of the Figure 2 caption. Moved to Methods, where
% all of it already appeared except the crop intervals, which were added:
% the 2,048 bp context, the 99th-percentile scaling mechanism (the resulting
% non-comparability caveat is KEPT, since it governs how the figure is read),
% "measuring predictive specificity against a uniform background", the
% 1-based-axis / 0-based-interval convention, the two crop intervals, and the
% clause that DR2 lies within the PmobK spacer -- that clause was NOT in
% Methods when this comment was written; it was added there on 2026-09-04.
...
\caption{
\textbf{Model-derived dependency patterns at a characterized origin of transfer and origin of replication.} Categorical-Jacobian dependency maps computed with frozen \model and cropped to the locus; each panel is scaled to its own crop, so intensities are not comparable between panels. Above each map: a logo track of per-position information content, and annotation lanes carrying externally sourced features as arrows (blue, inverted repeats; orange, direct repeats; green, promoter boxes). Red boxes mark an annotated feature's extent, not a model-detected boundary. Crops, coordinate conventions and feature sourcing are in \nameref{sec:methods}. \textbf{a}, Origin of transfer of \emph{Klebsiella pneumoniae} plasmid pIGRK (AY543071.1). IR1--IR3 and the 12 bp direct repeats DR1, DR2 are proposed in the source study, where the P\emph{mobK} $-$35 and $-$10 boxes are computational predictions rather than experimentally verified. IR4 is unannotated in that literature and was identified after inspecting this map. \textbf{b}, Minimal origin of replication of \emph{Escherichia coli} plasmid pSC101 (NC\_002056.1), cropped within the reported minimal origin, so the DnaA box and IHF site lie outside it. DR1--DR3 are the three 21 bp RepA iterons; IR1 and IR2 are the inverted repeats, containing the \emph{rep} promoter's $-$35 and $-$10 boxes respectively.
}
