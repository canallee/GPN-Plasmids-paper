...
\emph{oriT} references were compiled from three sources: the oriTDB2
% TODO cite: oriTDB2.
database, six consensus \emph{oriT} sequences from an \emph{oriT}-diversity
study,
% TODO cite: the oriT-diversity study (six consensus oriTs).
and 87 experimentally validated \emph{oriT}s from an
\emph{oriT}-transfer-network study.
% TODO cite: the oriT-transfer-network study (87 validated oriTs).
...
\textbf{112} by MMseqs2
% TODO cite: MMseqs2.
clustering (minimum sequence identity 90\%, coverage 90\%, coverage-mode =
shorter). \emph{oriV} references (452 sequences from the OriV-Finder
database) were deduplicated identically to \textbf{224} sequences.

\subsection*{Plasmid databases and origin detection}

Origins were located in two plasmid collections: PLSDB
% TODO cite: PLSDB.
(filtered to $\geq$ 2,000 bp) and IMG/PR \cite{camargo2024img}.
...
aligned to plasmid genomes with BLASTN (word size 7, dust filtering off),
...
% TODO cite: Pyrodigal-gv.
in metagenomic mode; intergenic regions were the complement of all CDS
...
with EMBOSS Water
% TODO cite: EMBOSS Water / Smith-Waterman.
(Smith--Waterman, gap open 10, gap extend 0.5) on both the forward and
...
50\% coverage set to the maximum distance. GraphPart
% TODO cite: GraphPart.
partitioned the origins into five parts at a similarity threshold of 0.3 with
...
engineered Addgene
% TODO cite: Addgene.
plasmids) served as a negative control. PlasmidGPT token hidden states were
...
used AdamW
% TODO cite: AdamW.
(learning rate $5\times10^{-4}$, weight decay 0.01) with cosine annealing
...
probabilities by isotonic regression
% TODO cite: isotonic regression / calibration.
...
% NOTE: the artifact titles this subsection simply "Inference", which collides
% with the pretraining Methods' own "Inference" heading above. Kept verbatim
% rather than renamed; disambiguate at the author's discretion.
