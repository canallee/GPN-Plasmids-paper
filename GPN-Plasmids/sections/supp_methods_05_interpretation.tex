...
PlasAnn \cite{islam2026plasann},
% Citation inserted 2026-09-04. The entry came from the artifact's own ref.bib
% but the artifact carried no marker at this sentence, so it was previously
% left as a TODO. It is the tool actually used, so it is now cited.
...
Interval operations use bioframe \cite{open2c2024bioframe}.
...
\cite{sugiura1993minimal,stenzel1987integration,nowak2021molecular} ---
% NOTE (2026-09-04): Sugiura 1993 maps the pSC101 minimal origin, the iterons
% and IR-1/IR-2; Stenzel 1987 maps the IHF site, which Sugiura does not, so
% both are needed. CAVEAT FOR AUTHOR: neither primary paper uses the labels
% DR1-DR3 / IR1 / IR2 that the figure carries. If those come from a GenBank or
% SnapGene annotation of NC_002056.1, say so in the caption rather than
% attributing the nomenclature to these papers.
...
% Sentence added 2026-09-04. The two main-figure crop intervals previously
% appeared ONLY in the Figure 4 caption, which was cut from 261 words. They
% are recorded here so nothing is lost.
The displayed crops are pIGRK (AY543071.1) [904,1214) and pSC101
(NC\_002056.1) [5100,5272); the pSC101 crop lies within the reported minimal
origin, so the DnaA box and the IHF site fall outside it. At pIGRK the second
direct repeat, DR2, lies within the spacer of the predicted P\emph{mobK}
promoter.
...
sequence \cite{tomita2005genetic,kishida2017host,llosa1991structural}.
% pTA1040: meijer1998rolling is the primary sequence source for the plasmid,
% but it reports oriT/RSA-like sequences for pTA1015 and pTA1060, NOT for
% pTA1040, and pTA1040 is absent from oriTDB. The manuscript presents pTA1040
% as a single-strand origin rather than an oriT, which is consistent, so the
% citation is attached below at the pTA1040 sentence, not here.
% TODO cite: PA83. Could not be verified as a plasmid oriT source. In the
% literature PA83 is a Pseudomonas aeruginosa STRAIN carrying a type IV
% CRISPR-Cas system, which matches how this manuscript uses it -- as a CRISPR
% array locus, not an origin. Supply the source you took the array call from.
...
\cite{couvin2018crisprcasfinder}
...
protein and genomic language models \cite{zhang2024protein,cornman2025omg}.
...
\cite{mcinnes2018umap,vanassel2025torchdr}
...
elements and for Shine--Dalgarno sequences, derived from RegulonDB
\cite{salgado2024regulondb}, using FIMO \cite{grant2011fimo} of the MEME
Suite \cite{bailey2015meme} as implemented in pymemesuite
\cite{larralde2022pymemesuite}.
...
mean information content reported for each promoter panel was computed
over a 300 bp scan interval, not over the displayed crop.
% Corrected 2026-09-04: this said "quoted in each promoter panel title", but
% figS3_promoter.pdf has no panel titles -- the values live only in the
% supplementary caption, and panel d carries no value at all. Either supply the
% value for panel d or say why it is absent.

ViennaRNA \cite{lorenz2011viennarna}
% Cited 2026-09-04. The artifact's note said to cite it only if an analysis
% using it is reported. This sentence does report its use in the project while
% stating that no displayed panel depends on it, so naming the software
% without attribution would be the worse of the two options.
...
single-strand origin \cite{meijer1998rolling} is therefore not presented as a
structural or
...
% Added 2026-09-04. This paragraph carries facts that previously appeared ONLY
% in the Results prose, which was trimmed to the Nature Biotechnology main-text
% limit. Nothing here is new: the stem-loop search, the interval coordinates,
% the loop sequence and the shuffled-null result are transcribed from
% 05_interpretation.tex as they stood.
\subsection*{Uncurated interval search and its null}

The unannotated pIGRK interval reported in the Results, [1119,1152), was
identified after inspecting the dependency map, by a stem-loop search over the
surrounding sequence rather than by a prespecified detector. Its arms match at
9 of 10 positions and the match degrades on extension, and its 13 bp loop,
\texttt{GAGCGATAGCGAA}, contains two of the four 5$'$-AGCGA-3$'$ motifs that
the source study proposes as MobK recognition coordinates. Because the search
was post hoc, the arm-identity score was tested against a null of shuffled
sequence; it does not reach significance (\emph{P} $\approx$ 0.08). The
interval is therefore reported as an unconfirmed hypothesis, not as a
discovery, and no multiple-testing correction is applicable to a single post
hoc interval.
